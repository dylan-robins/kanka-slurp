...
class KankaSlurp:
        ...
        while True:
            ...
            try:
                ...
                if resp is not None:
                    ...
                    self.logger.exception(f"HTTP error for {url}: {e}")
                raise
            except requests.RequestException as e:
                self.logger.exception(f"Request failed for {url}: {e}")
                raise
    ...
    def download_image(self, url: str, subdir: str = "files") -> Optional[str]:
        ...
        try:
            ...
            if content_type and content_type not in ALLOWED_MIME_TYPES:
                self.logger.warning(
                    f"Skipping image with invalid MIME type {content_type}: {url}"
                )
                return None
            ...
            if content_length:
                try:
                    ...
                    if size > MAX_IMAGE_SIZE:
                        self.logger.warning(
                            f"Skipping oversized image ({size} bytes): {url}"
                        )
                        return None
                except ValueError:
                    pass
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to download image: {resp.status_code} {url}"
                )
                return None
                ...
                for chunk in resp.iter_content(1024 * 64):
                    if chunk:
                        ...
                        if downloaded > MAX_IMAGE_SIZE:
                            self.logger.error(
                                f"File exceeded max size during download: {url}"
                            )
                            out_file.unlink()
                            return None
    ...
    def _determine_entity_type(
        ...
        t = (payload.get("type") if isinstance(payload, dict) else None) or item.get(
            "type"
        )
        if t:
            return str(t).lower()
    ...
    def fetch_items_details(
        self, endpoint: str, items: List[Dict[str, Any]], update_mode: bool = False
    ):
        ...
        for it in tqdm(items, desc=f"Details {endpoint}"):
            ...
            if update_mode:
                ...
                if (
                    ...
                    and str(local_updated_at) == str(server_updated_at)
                ):
            ...
            try:
                ...
                if api_url and api_url.startswith("http"):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Detail HTTP error for {api_url}: {resp.status_code}"
                        )
                        continue
                    data = resp.json()
                else:
                ...
                if not isinstance(payload, dict):
                    self.logger.warning(
                        f"Unexpected payload format for {endpoint}/{item_id}"
                    )
                    continue
                ...
                if (
                    image_full
                    and isinstance(image_full, str)
                    and image_full.startswith("http")
                ):
                ...
                # Save Markdown
                md_rel = self._save_item_markdown(
                    endpoint, item_id, html_to_save, metadata
                )
                ...
                self.logger.error(
                    f"Failed to fetch detail for {endpoint}/{item_id}: {e}"
                )
    ...
    def slurp(self, update: bool = False):
        ...
        try:
            ...
            if update and entities:
                summary["updated"] = self.fetch_items_details(
                    "entities", entities, update_mode=True
                )
            elif entities:
        ...
        self.logger.info(
            "Skipping discovery of other endpoints; exported `entities` only."
        )
